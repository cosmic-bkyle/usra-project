...
def train(parquet="dr_to_solved/labelled_drs.parquet", epochs=3, bs=1024, device="cpu"):
    ...
    for ep in range(epochs): #train in epochs, default 3.
        ...
        for x,y in train_dataloader:
            ...
            loss.backward()
            opt.step()
        print(f"epoch {ep:02d} | train loss {running_loss/seen:.4f} val‑MAE {mae(net,val_dataloader,device):.3f}")
...
def main():
    ...
    '''
    - print loss as well as error
    - just give the features that I engineered, no permutation
    - convolutional networks - guess a good architecture, encode neighboring relationships
    ''' 
